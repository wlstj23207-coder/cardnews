...
> **v6.0** — 14종 슬라이드 타입 + 8종 템플릿 스타일 + 팀 토론 파이프라인 + **이미지 크롤러 (Google/Unsplash 웹)**

## 프로젝트 개요

이 프로젝트는 주어진 주제에 대해 Instagram 카드뉴스(캐러셀 포스트)를 자동으로 생성합니다.
Claude Code가 오케스트레이터 역할을 하며, 리서치 → **리서치 검증 (팀 토론)** → 카피라이팅 → **카피 토론 (팀 토론)** → **이미지 크롤링** → 렌더링 → 검토 파이프라인을 실행합니다.
...
| `bg_photo_mode` | `auto` | 배경사진 적용 방식: `auto`(주제 기반 자동 크롤링) / `cover-only`(표지만) / `all`(모든 슬라이드) / `none`(사용 안 함) |
...
- **이미지가 필요한 슬라이드는** `content-image` 타입 (image_url은 비워두면 크롤러가 자동 할당)
...
4. 오케스트레이터가 두 에이전트의 피드백을 종합:
   ...
   - 양쪽 모두 통과 → **이미지 크롤링** 단계로 진행

**통과 기준**: 후킹 점수 7점 이상 + 카피 에디터의 주요 지적사항 0건

---

### Step 3.5 내: 이미지 크롤링 (Crawler)

카피 토론 통과 후, **이미지 크롤러**를 사용하여 주제와 관련된 배경사진을 자동으로 크롤링하고 슬라이드에 할당합니다. API 키 설정 없이 즉시 사용 가능합니다.

---

##### 🕷️ 배경사진 크롤링 방식

이미지 크롤러는 두 가지 방식을 지원합니다:

**1. Unsplash Source API (기본, 추천)**
- API 키 없이 랜덤 이미지를 가져옵니다.
- 안정적이고 빠릅니다.
- URL: `https://source.unsplash.com/random/?{keyword}&w=1080&h=1350&fit=crop`

**2. Unsplash 웹 크롤링**
- Unsplash 웹사이트 검색 결과에서 이미지 URL을 추출합니다.
- HTML 파싱으로 이미지를 가져옵니다.
...
- 레퍼런스 이미지(도시 사진 + 하단 텍스트)처럼 하단에 텍스트 집중
...
- **캐시 키**: `{source}_{keyword}_{orientation}_{count}_{page}`
- **캐시 위치**: `.cache/images/`
- **캐시 유효기간**: 24시간
- **캐시 사용**: 기본 활성화 (`useCache: false`로 비활성화 가능)
...
[
  {
    ...
    "image_url": "https://source.unsplash.com/random/?Korean%20woman%20dating%2030s&w=1080&h=1350&fit=crop",
  ...
  },
  {
    ...
    "image_url": "https://source.unsplash.com/random/?Osaka%20Dotonbori%20travel&w=1080&h=1350&fit=crop",
    "image_credit": "Unsplash",
    "image_color": "#888888"
  }
]
...
| 상황 | 처리 방법 |
|---|---|
| 크롤링 실패 | 해당 슬라이드는 배경사진 없이 기본 템플릿 색상으로 렌더링 |
| 캐시 오류 | 직접 크롤링으로 폴백 |

**주의**: 크롤러가 반환하는 URL은 외부 URL이므로 렌더링 환경에서 네트워크 접근이 가능해야 합니다. Puppeteer 렌더러는 기본적으로 외부 이미지를 로드합니다.
...
- `image_crawling.source`: 이미지 크롤링 소스 (`unsplash-source` / `unsplash-web`)
...
│   ├── image-crawler.js # 이미지 크롤러 (새로운 크롤링 방식)
│   └── generate-samples.js
